param.vec.2.ads_TIRT_3_pick <- function(pv,J,D,sigm=NULL,fix.sigma=FALSE,blocksize=3){
...
data.sim_TIRT_3_pick <- function(item.par,theta,BID){

  Y <- matrix(NA,nrow = nrow(theta),ncol = max(BID$Block))
  for(j in unique(BID$Block)){
    pj <- Pj_TIRT_3_pick(j=j,theta=theta,BID=BID,item.par=item.par)
...
kernel_TIRT_3_pick <- function(Y,BID,a,d,sigm,theta,B=20,J,D,N,cor.matrix=FALSE,positive,fix.sigma=FALSE,blocksize=3,hessian=FALSE){
# posterior for sampling theta - see Equation 4

  logLi_TIRT_3_pick <- function(thetai, #vector of length D  ability for person i
                               a,#matrix 3 x J
                               d,#matrix 3 x J
                               yi,# vector of J
                               BID,
                               prior=TRUE,
                               sigma=NULL){
    ll <- sum(log(P.Yi_TIRT_3_pick(a=a,d=d,theta=matrix(thetai[BID$Dim],nrow = 3),yi=yi)))
    lprior <- 0
    if(prior)
...
arm.sampler_TIRT_3_pick <- function(thetai,p){
  logLi_TIRT_3_pick(thetai, a=a, d=d,yi=Y[i,], BID=BID,prior=TRUE,sigma=sigm)
}
# function to be optimized for estimating item parameters - see Equation 8
objfun_TIRT_3_pick <- function(x,thetaj,yj,blocksize=3){
  IN <- 2*blocksize-1 # Number of item parameter
  -1*P.Yj_TIRT_3_pick(aj=(x[1:blocksize]),dj=c(x[(blocksize+1):IN],-1*sum(x[(blocksize+1):IN])),thetaj,yj)

}


P.Yj_TIRT_3_pick <- function(aj,#vector of length 3
  ...
  pbca=p21*p31*p23
  pcba=p21*p31*p32
  prenorm=rbind(pabc+pacb,pbac+pbca,pcab+pcba)
  sumprenorm=matrix(colSums(prenorm)[col(prenorm)],nrow = 3)
...
P.Yi_TIRT_3_pick <- function(a,#matrix 3 x J
                            d,#matrix 3 x J
  ...
  for(b in seq_len(B)){
    ...
    theta <- foreach (i=seq_len(N),.combine="rbind",.errorhandling = "stop",.inorder = TRUE)%dopar%{
                          ...
                          armspp::arms_gibbs(n_samples = 1,previous = theta[i,],log_pdf = arm.sampler_TIRT_3_pick,
    ...
    opt <- foreach (j=1:J,.combine="cbind",.errorhandling = "stop",.inorder = TRUE)%dopar%{
                      ...
                      optim(par=c(a[,j],d[1:(blocksize-1),j]),fn = objfun_TIRT_3_pick,method = "L-BFGS-B",lower = lo, upper = up,
    ...
    if(hessian==TRUE){
      hess_vec=foreach (j=1:J,.combine="cbind",.errorhandling = "stop",.inorder = TRUE)%dopar%{
        lo <- c(ifelse(positive[which(BID$Block==j)]>0,c(0,0,0),c(-Inf,-Inf,-Inf)),-6,-6)
        up <- c(ifelse(positive[which(BID$Block==j)]>0,c(Inf,Inf,Inf),c(0,0,0)),6,6)
        optim(par=c(a[,j],d[1:2,j]),fn = objfun_TIRT_3_pick,method = "L-BFGS-B",lower = lo, upper = up,
...
Pj_TIRT_3_pick <- function(j,theta,aj=NULL,dj=NULL,BID=NULL,item.par=NULL){
...
P.Yi_TIRT_3_pick <- function(a,#matrix 3 x J
  ...
  logL_TIRT_3_pick <- function(theta, #vector of length D  ability for person i
                               a,#matrix 3 x J
                               d,#matrix 3 x J
                               yi,# vector of J
                               BID,
