...
singleI_2PL_4_pick <- function(theta,item.par,BID,Y,model,I,h=NULL){
  ...
  XPDEss_item_2PL_4_pick <- function(theta,a,d,Y){
    all.x <- list(1,2,3,4)
    g <- grad_item_2PL_4_pick(theta,a,d,x=all.x[[Y]])
    ret <- tcrossprod(g,g)/px_2PL_4_pick(theta,a,d,xx=all.x[[Y]])^2

    ret
  }
  FDMEss_item_2PL_4_pick <- function(theta,a,d,Y,h=1e-5){
    all.x <- list(1,2,3,4)
    g <- grad_FDM_item_2PL_4_pick(theta,a,d,x=all.x[[Y]],h=h)
    ret <- tcrossprod(g,g)/px_2PL_4_pick(theta,a,d,xx=all.x[[Y]])^2

    ret
  }
  RESEss_item_2PL_4_pick <- function(theta,a,d,Y,h=1){
    all.x <- list(1,2,3,4)
    g <- grad_RES_item_2PL_4_pick(theta,a,d,x=all.x[[Y]],h=h)
    ret <- tcrossprod(g,g)/px_2PL_4_pick(theta,a,d,xx=all.x[[Y]])^2

    ret
  }
  CDMEss_item_2PL_4_pick <- function(theta,a,d,Y,h=1){
    all.x <- list(1,2,3,4)
    g <- grad_CDM_item_2PL_4_pick(theta,a,d,x=all.x[[Y]],h=h)
    ret <- tcrossprod(g,g)/px_2PL_4_pick(theta,a,d,xx=all.x[[Y]])^2
  ...
  FDMSS_item_2PL_4_pick <- function(theta,a,d,Y,h=1e-5){
    all.x <- list(1,2,3,4)
    g <- grad_FDM_item_2PL_4_pick(theta,a,d,x=all.x[[Y]],h=h)
    g
  }
  RESSS_item_2PL_4_pick <- function(theta,a,d,Y,h=1){
    all.x <- list(1,2,3,4)
    g <- grad_RES_item_2PL_4_pick(theta,a,d,x=all.x[[Y]],h=h)
    g
  }
  CDMSS_item_2PL_4_pick <- function(theta,a,d,Y,h=1){
    all.x <- list(1,2,3,4)
    g <- grad_CDM_item_2PL_4_pick(theta,a,d,x=all.x[[Y]],h=h)
    g
  }
  XPDSS_item_2PL_4_pick <- function(theta,a,d,Y){
    all.x <- list(1,2,3,4)
    g <- grad_item_2PL_4_pick(theta,a,d,x=all.x[[Y]])
    g
  }
  ...
  itemI_2PL_4_pick <- function(theta,item.par,BID,Y,I,h=NULL){
    ...
    for(b in unique(BID$Block)){
      ...
      Y.b <- Y[b]
      if(I=="XPD"){
        itemIEss[item,item] <- XPDEss_item_2PL_4_pick(theta.b,a.b,d.b,Y.b)
        itemISS[item] <- XPDSS_item_2PL_4_pick(theta.b,a.b,d.b,Y.b)
      }else if(I=="FDM"){
        if(is.null(h)){
          itemIEss[item,item] <- FDMEss_item_2PL_4_pick(theta.b,a.b,d.b,Y.b)
          itemISS[item] <- FDMSS_item_2PL_4_pick(theta.b,a.b,d.b,Y.b)
        }else{
          itemIEss[item,item] <- FDMEss_item_2PL_4_pick(theta.b,a.b,d.b,Y.b,h=h)
          itemISS[item] <- FDMSS_item_2PL_4_pick(theta.b,a.b,d.b,Y.b,h=h)
        }
      }else if(I=="CDM"){
        if(is.null(h)){
          itemIEss[item,item] <- CDMEss_item_2PL_4_pick(theta.b,a.b,d.b,Y.b)
          itemISS[item] <- CDMSS_item_2PL_4_pick(theta.b,a.b,d.b,Y.b)
        }else{
          itemIEss[item,item] <- CDMEss_item_2PL_4_pick(theta.b,a.b,d.b,Y.b,h=h)
          itemISS[item] <- CDMSS_item_2PL_4_pick(theta.b,a.b,d.b,Y.b,h=h)
        }
      }else if(I=="RES"){
        if(is.null(h)){
          itemIEss[item,item] <- RESEss_item_2PL_4_pick(theta.b,a.b,d.b,Y.b)
          itemISS[item] <- RESSS_item_2PL_4_pick(theta.b,a.b,d.b,Y.b)
        }else{
          itemIEss[item,item] <- RESEss_item_2PL_4_pick(theta.b,a.b,d.b,Y.b,h=h)
          itemISS[item] <- RESSS_item_2PL_4_pick(theta.b,a.b,d.b,Y.b,h=h)
        }
      }
    }
  ...
  sinI_matrix=foreach(i = seq_len(nrow(theta)),.inorder=FALSE,.combine='+',.export = c("foreach","%dopar%","px_2PL_4_pick"))%dopar%{
                                                                                         itemI_2PL_4_pick(theta[i,],item.par,BID,Y[i,],I,h=h)$itemIEss
                                                                                       }
  sinI_vector=foreach(i = seq_len(nrow(theta)),.inorder=FALSE,.combine='+',.export = c("foreach","%dopar%","px_2PL_4_pick"))%dopar%{
                                                                                         itemI_2PL_4_pick(theta[i,],item.par,BID,Y[i,],I,h=h)$itemISS
                                                                                       }
  list(sinI_matrix=sinI_matrix,sinI_vector=sinI_vector)
}

itemSE_2PL_4_pick <- function(x,BID,Y,model,SE,IN,theta,h=NULL){


  matrix=singleI_2PL_4_pick(theta=theta,item.par=data.frame(c(x$a),c(x$d)),BID=BID,Y=Y,I=SE,model=model,h=h)$sinI_matrix
  vector=singleI_2PL_4_pick(theta=theta,item.par=data.frame(c(x$a),c(x$d)),BID=BID,Y=Y,I=SE,model=model,h=h)$sinI_vector
