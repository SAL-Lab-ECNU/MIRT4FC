...
#'
#'
logLi_4_mole <- function(thetai, #vector of length D  ability for person i
                  a,#matrix 4 x J
                  d,#matrix 4 x J
                  yi,# vector of J
                  BID,
                  prior=TRUE,
                  sigma=NULL){
  ll <- sum(log(P.Yi_4_mole(a=a,d=d,theta=matrix(thetai[BID$Dim],nrow = 4),yi=yi)))
  lprior <- 0
  if(prior)
...
#'              12 alternatives of block j by all N persons: 12 x N matrix
...
#'
#'
Pj_4_mole <- function(j,theta,aj=NULL,dj=NULL,BID=NULL,item.par=NULL){
...
#'
#'
data.sim_4_mole <- function(item.par,theta,BID){

  Y <- matrix(NA,nrow = nrow(theta),ncol = max(BID$Block))
  for(j in unique(BID$Block)){
    pj <- Pj_4_mole(j=j,theta=theta,BID=BID,item.par=item.par)
...
#'
#'
kernel_4_mole <- function(Y,BID,a,d,sigm,theta,B=20,J,D,N,cor.matrix=FALSE,positive,fix.sigma=FALSE){
# posterior for sampling theta - see Equation 4
arm.sampler_4_mole <- function(thetai,p){
  logLi_4_mole(thetai, a=a, d=d,yi=Y[i,], BID=BID,prior=TRUE,sigma=sigm)
}
# function to be optimized for estimating item parameters - see Equation 8
objfun_4_mole <- function(x,thetaj,yj){
  -1*P.Yj_4_mole(aj=(x[1:4]),dj=c(x[5:7],-sum(x[5:7])),thetaj,yj)
}
  ...
  for(b in seq_len(B)){
    ...
    theta <- foreach (i=seq_len(N),.combine="rbind",.errorhandling = "stop",.inorder = TRUE,
                      .export = c("arm.sampler_4_mole","logLi_4_mole","P.Yi_4_mole","a","d","Y","BID","sigm"))%dopar%{

                        armspp::arms_gibbs(n_samples = 1,previous = theta[i,],log_pdf = arm.sampler_4_mole,
    ...
    opt <- foreach (j=1:J,.combine="cbind",.errorhandling = "stop",.inorder = TRUE,
                    .export = c("objfun_4_mole","P.Yj_4_mole","a","d","Y","BID","theta"))%dopar%{
                      ...
                      optim(par=c(a[,j],d[1:3,j]),fn = objfun_4_mole,method = "L-BFGS-B",lower = lo, upper = up,
...
#'
#'
P.Yj_4_mole <- function(aj,#vector of length 4
...
#'
#'
P.Yi_4_mole <- function(a,#matrix 4 x J
                 d,#matrix 4 x J
...
#'
#'
param.vec.2.ads_4_mole <- function(pv,J,D,sigm=NULL,fix.sigma=FALSE){
