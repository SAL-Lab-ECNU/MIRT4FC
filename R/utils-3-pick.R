...
#'
#'
logLi_3_pick <- function(thetai, #vector of length D  ability for person i
                    a,#matrix 3 x J
                    d,#matrix 3 x J
                    yi,# vector of J
                    BID,
                    prior=TRUE,
                    sigma=NULL){
  ll <- sum(log(P.Yi_3_pick(a=a,d=d,theta=matrix(thetai[BID$Dim],nrow = 3),yi=yi)))
  lprior <- 0
  if(prior)
...
#'              3 alternatives of block j by all N persons: 3 x N matrix
...
#'
#'
Pj_3_pick <- function(j,theta,aj=NULL,dj=NULL,BID=NULL,item.par=NULL){
...
#'
#'
data.sim_3_pick <- function(item.par,theta,BID){

  Y <- matrix(NA,nrow = nrow(theta),ncol = max(BID$Block))
  for(j in unique(BID$Block)){
    pj <- Pj_3_pick(j=j,theta=theta,BID=BID,item.par=item.par)
...
#'
#'
kernel_3_pick <- function(Y,BID,a,d,sigm,theta,B=20,J,D,N,cor.matrix=FALSE,positive,fix.sigma=FALSE){
# posterior for sampling theta - see Equation 4
arm.sampler_3_pick <- function(thetai,p){
  logLi_3_pick(thetai, a=a, d=d,yi=Y[i,], BID=BID,prior=TRUE,sigma=sigm)
}
# function to be optimized for estimating item parameters - see Equation 8
objfun_3_pick <- function(x,thetaj,yj){
  -1*P.Yj_3_pick(aj=(x[1:3]),dj=c(x[4:5],-sum(x[4:5])),thetaj,yj)
}
  ...
  for(b in seq_len(B)){
    ...
    theta <- foreach (i=seq_len(N),.combine="rbind",.errorhandling = "stop",.inorder = TRUE,
                      .export = c("arm.sampler_3_pick","logLi_3_pick","P.Yi_3_pick","a","d","Y","BID","sigm"))%dopar%{

                          armspp::arms_gibbs(n_samples = 1,previous = theta[i,],log_pdf = arm.sampler_3_pick,
    ...
    opt <- foreach (j=1:J,.combine="cbind",.errorhandling = "stop",.inorder = TRUE,
                    .export = c("objfun_3_pick","P.Yj_3_pick","a","d","Y","BID","theta"))%dopar%{
                      ...
                      optim(par=c(a[,j],d[1:2,j]),fn = objfun_3_pick,method = "L-BFGS-B",lower = lo, upper = up,
...
#'
#'
P.Yj_3_pick <- function(aj,#vector of length 3
...
#'
#'
P.Yi_3_pick <- function(a,#matrix 3 x J
                 d,#matrix 3 x J
...
#'
#'
param.vec.2.ads_3_pick <- function(pv,J,D,sigm=NULL,fix.sigma=FALSE){
