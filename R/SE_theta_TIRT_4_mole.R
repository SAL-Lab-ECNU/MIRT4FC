px_TIRT_4_mole <- function(theta,a,d,xx=c(1,2,3,4)){ #a=c(ai,aj,ak) d=c(di,dj,dk) theta=c(thetai,thetaj,thetak)
  alphatheta=a*theta
...
grad_theta_TIRT_4_mole <- function(theta,a,d,x=c(1,2,3,4),h=1e-5){#一个block内的一阶导，#a=c(ai,aj,ak) d=c(di,dj,dk) theta=c(thetai,thetaj,thetak)

  g.i <- (px_TIRT_4_mole(theta+c(h,0,0,0),a,d,x)-px_TIRT_4_mole(theta,a,d,x))/h
  g.j <- (px_TIRT_4_mole(theta+c(0,h,0,0),a,d,x)-px_TIRT_4_mole(theta,a,d,x))/h
  g.k <- (px_TIRT_4_mole(theta+c(0,0,h,0),a,d,x)-px_TIRT_4_mole(theta,a,d,x))/h
  g.l <- (px_TIRT_4_mole(theta+c(0,0,0,h),a,d,x)-px_TIRT_4_mole(theta,a,d,x))/h
...
Ib_theta_TIRT_4_mole <- function(theta,a,d){
  ...
  for(x in 1:length(all.x)){
    g <- grad_theta_TIRT_4_mole(theta,a,d,x=all.x[[x]])
    ret <- ret + tcrossprod(g,g)/px_TIRT_4_mole(theta,a,d,xx=all.x[[x]])
  }
  ret
}


test.info_theta_TIRT_4_mole <- function(theta,item.par,BID){
  ...
  for(b in unique(BID$Block)){
    ...
    theta.b <- theta[dim.b]
    complete.info.b[dim.b,dim.b] <- info[[b]] <- Ib_theta_TIRT_4_mole(theta.b,a.b,d.b)
    I <- I+complete.info.b
