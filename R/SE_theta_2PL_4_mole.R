px_2PL_4_mole <- function(theta,a,d,xx=c(1,2,3,4)){ #a=c(ai,aj,ak) d=c(di,dj,dk) theta=c(thetai,thetaj,thetak)
  z <- exp(a*theta-d)
...
Ib_theta_2PL_4_mole <- function(theta,a,d){
  ...
  for(x in 1:length(duiying)){
    ...
    mid1 <- tcrossprod(g,g)/px_2PL_4_mole(theta,a,d,xx=all.x[[duiying[x,1]]])-hes_theta_2PL_4_mole(theta,a,d,x=all.x[[duiying[x,1]]])
    g2 <- grad_theta_2PL_4_mole(theta,a,d,x=all.x[[duiying[x,2]]])
    mid2 <- tcrossprod(g,g)/px_2PL_4_mole(theta,a,d,xx=all.x[[duiying[x,2]]])-hes_theta_2PL_4_mole(theta,a,d,x=all.x[[duiying[x,2]]])
    ret <- ret + mid1+mid2
  }
  ret
}
