px_2PL_4_pick <- function(theta,a,d,x=1){ #a=c(ai,aj,ak) d=c(di,dj,dk) theta=c(thetai,thetaj,thetak)
  z <- exp(a*theta-d)
  prod(z[x])/sum(z)
}
...
Ib_theta_2PL_4_pick <- function(theta,a,d){
  ...
  for(x in 1:length(all.x)){
    ...
    ret <- ret + tcrossprod(g,g)/px_2PL_4_pick(theta,a,d,x=all.x[[x]])-hes_theta_2PL_4_pick(theta,a,d,x=all.x[[x]])
  }
  ret
}
