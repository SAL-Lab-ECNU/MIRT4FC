...
Pj_TIRT_3_rank <- function(j,theta,aj=NULL,dj=NULL,BID=NULL,item.par=NULL){
  ...
  pbca=p21*p31*p23
  pcba=p21*p31*p32
  prenorm=rbind(pabc,pacb,pbac,pcab,pbca,pcba)
  sumprenorm=matrix(colSums(prenorm)[col(prenorm)],nrow = 6)
  prenorm/sumprenorm
}

data.sim_TIRT_3_rank <- function(item.par,theta,BID){

  Y <- matrix(NA,nrow = nrow(theta),ncol = max(BID$Block))
  for(j in unique(BID$Block)){
    pj <- Pj_TIRT_3_rank(j=j,theta=theta,BID=BID,item.par=item.par)
...
kernel_TIRT_3_rank <- function(Y,BID,a,d,sigm,theta,B=20,J,D,N,cor.matrix=FALSE,positive,fix.sigma=FALSE,hessian=FALSE){
# posterior for sampling theta - see Equation 4
arm.sampler_TIRT_3_rank <- function(thetai,p){
  logLi_TIRT_3_rank(thetai, a=a, d=d,yi=Y[i,], BID=BID,prior=TRUE,sigma=sigm)
}
# function to be optimized for estimating item parameters - see Equation 8
objfun_TIRT_3_rank <- function(x,thetaj,yj){
  -1*P.Yj_TIRT_3_rank(aj=(x[1:3]),dj=c(x[4:5],-sum(x[4:5])),thetaj,yj)
}

logLi_TIRT_3_rank <- function(thetai, #vector of length D  ability for person i
                             a,#matrix 3 x J
                             d,#matrix 3 x J
                             yi,# vector of J
                             BID,
                             prior=TRUE,
                             sigma=NULL){
  ll <- sum(log(P.Yi_TIRT_3_rank(a=a,d=d,theta=matrix(thetai[BID$Dim],nrow = 3),yi=yi)))
  lprior <- 0
  if(prior)
...
P.Yj_TIRT_3_rank <- function(aj,#vector of length 3
...
P.Yi_TIRT_3_rank <- function(a,#matrix 3 x J
                            d,#matrix 3 x J
  ...
  for(b in seq_len(B)){
    ...
    theta <- foreach (i=seq_len(N),.combine="rbind",.errorhandling = "stop",.inorder = TRUE)%dopar%{
                        ...
                        armspp::arms_gibbs(n_samples = 1,previous = theta[i,],log_pdf = arm.sampler_TIRT_3_rank,
    ...
    opt <- foreach (j=1:J,.combine="cbind",.errorhandling = "stop",.inorder = TRUE)%dopar%{
                      ...
                      optim(par=c(a[,j],d[1:2,j]),fn = objfun_TIRT_3_rank,method = "L-BFGS-B",lower = lo, upper = up,
...
param.vec.2.ads_TIRT_3_rank <- function(pv,J,D,sigm=NULL,fix.sigma=FALSE){
...
logL_TIRT_3_rank <- function(theta, #vector of length D  ability for person i
                             a,#matrix 3 x J
                             d,#matrix 3 x J
                             yi,# vector of J
                             BID,
