...
Pj_TIRT_4_pick <- function(j,theta,aj=NULL,dj=NULL,BID=NULL,item.par=NULL){
  ...
  prenorm=rbind(pabcd+pabdc+pacbd+pacdb+padbc+padcb,
                pbacd+pbadc+pbcad+pbcda+pbdac+pbdca,
                pcabd+pcadb+pcbad+pcbda+pcdab+pcdba,
                pdabc+pdacb+pdbac+pdbca+pdcab+pdcba)
  sumprenorm=matrix(colSums(prenorm)[col(prenorm)],nrow = 4)
  prenorm/sumprenorm
}

data.sim_TIRT_4_pick <- function(item.par,theta,BID){

  Y <- matrix(NA,nrow = nrow(theta),ncol = max(BID$Block))
  for(j in unique(BID$Block)){
    pj <- Pj_TIRT_4_pick(j=j,theta=theta,BID=BID,item.par=item.par)
...
kernel_TIRT_4_pick <- function(Y,BID,a,d,sigm,theta,B=20,J,D,N,cor.matrix=FALSE,positive,fix.sigma=FALSE,hessian=FALSE){
# posterior for sampling theta - see Equation 4
arm.sampler_TIRT_4_pick <- function(thetai,p){
  logLi_TIRT_4_pick(thetai, a=a, d=d,yi=Y[i,], BID=BID,prior=TRUE,sigma=sigm)
}
# function to be optimized for estimating item parameters - see Equation 8
objfun_TIRT_4_pick <- function(x,thetaj,yj){
  -1*P.Yj_TIRT_4_pick(aj=(x[1:4]),dj=c(x[5:7],-sum(x[5:7])),thetaj,yj)
}
P.Yj_TIRT_4_pick <- function(aj,#vector of length 4
...
P.Yi_TIRT_4_pick <- function(a,#matrix 4 x J
                            d,#matrix 4 x J
...
logLi_TIRT_4_pick <- function(thetai, #vector of length D  ability for person i
                             a,#matrix 4 x J
                             d,#matrix 4 x J
                             yi,# vector of J
                             BID,
                             prior=TRUE,
                             sigma=NULL){
  ll <- sum(log(P.Yi_TIRT_4_pick(a=a,d=d,theta=matrix(thetai[BID$Dim],nrow = 4),yi=yi)))
  lprior <- 0
  if(prior)
  ...
  for(b in seq_len(B)){
    ...
    theta <- foreach (i=seq_len(N),.combine="rbind",.errorhandling = "stop",.inorder = TRUE)%dopar%{
                        ...
                        armspp::arms_gibbs(n_samples = 1,previous = theta[i,],log_pdf = arm.sampler_TIRT_4_pick,
    ...
    opt <- foreach (j=1:J,.combine="cbind",.errorhandling = "stop",.inorder = TRUE)%dopar%{
                      ...
                      optim(par=c(a[,j],d[1:3,j]),fn = objfun_TIRT_4_pick,method = "L-BFGS-B",lower = lo, upper = up,
...
param.vec.2.ads_TIRT_4_pick <- function(pv,J,D,sigm=NULL,fix.sigma=FALSE){
...
logL_TIRT_4_pick <- function(theta, #vector of length D  ability for person i
                             a,#matrix 3 x J
                             d,#matrix 3 x J
                             yi,# vector of J
                             BID,
