px_item_2PL_4_mole <- function(theta,a,d,x=c(1,2,3,4)){#一个block内的一阶导，#a=c(ai,aj,ak) d=c(di,dj,dk) theta=c(thetai,thetaj,thetak)
  z <- exp(t(a*t(theta)-d))
  ...
  FDM_item_2PL_4_mole <- function(theta,a,d,Y,h=1e-5){
    ...
    g/(px_2PL_4_mole(theta,a,d,x=all.x[[duiying[Y,1]]])+px_2PL_4_mole(theta,a,d,x=all.x[[duiying[Y,2]]]))
  }
  RES_item_2PL_4_mole <- function(theta,a,d,Y,h=1){
    all.x <- list(c(1,2,3,4),c(1,2,4,3),c(1,3,2,4),c(1,3,4,2),c(1,4,2,3),c(1,4,3,2),
                  c(2,1,3,4),c(2,1,4,3),c(2,3,1,4),c(2,3,4,1),c(2,4,1,3),c(2,4,3,1),
                  c(3,1,2,4),c(3,1,4,2),c(3,2,1,4),c(3,2,4,1),c(3,4,1,2),c(3,4,2,1),
                  c(4,1,2,3),c(4,1,3,2),c(4,2,1,3),c(4,2,3,1),c(4,3,1,2),c(4,3,2,1))
  ...
  CDM_item_2PL_4_mole <- function(theta,a,d,Y,h=1){
    all.x <- list(c(1,2,3,4),c(1,2,4,3),c(1,3,2,4),c(1,3,4,2),c(1,4,2,3),c(1,4,3,2),
                  c(2,1,3,4),c(2,1,4,3),c(2,3,1,4),c(2,3,4,1),c(2,4,1,3),c(2,4,3,1),
                  c(3,1,2,4),c(3,1,4,2),c(3,2,1,4),c(3,2,4,1),c(3,4,1,2),c(3,4,2,1),
                  c(4,1,2,3),c(4,1,3,2),c(4,2,1,3),c(4,2,3,1),c(4,3,1,2),c(4,3,2,1))
  ...
  XPD_item_2PL_4_mole <- function(theta,a,d,Y){
    all.x <- list(c(1,2,3,4),c(1,2,4,3),c(1,3,2,4),c(1,3,4,2),c(1,4,2,3),c(1,4,3,2),
                  c(2,1,3,4),c(2,1,4,3),c(2,3,1,4),c(2,3,4,1),c(2,4,1,3),c(2,4,3,1),
                  c(3,1,2,4),c(3,1,4,2),c(3,2,1,4),c(3,2,4,1),c(3,4,1,2),c(3,4,2,1),
                  c(4,1,2,3),c(4,1,3,2),c(4,2,1,3),c(4,2,3,1),c(4,3,1,2),c(4,3,2,1))
    ...
    g/(px_item_2PL_4_mole(theta,a,d,x=all.x[[duiying[Y,1]]])+px_item_2PL_4_mole(theta,a,d,x=all.x[[duiying[Y,2]]]))
  }
  ...
  itemI_2PL_4_mole <- function(theta,item.par,BID,Y,I,h=NULL){
    ...
    for(b in unique(BID$Block)){
      ...
      Y.b <- Y[b]
      if(I=="XPD"){
        itemI[,item] <- XPD_item_2PL_4_mole(theta.b,a.b,d.b,Y.b)
      }else if(I=="FDM"){
        if(is.null(h)){
          itemI[item] <- FDM_item_2PL_4_mole(theta.b,a.b,d.b,Y.b)
        }else{
          itemI[item] <- FDM_item_2PL_4_mole(theta.b,a.b,d.b,Y.b,h=h)
        }
      }else if(I=="CDM"){
        if(is.null(h)){
          itemI[item] <- CDM_item_2PL_4_mole(theta.b,a.b,d.b,Y.b)
        }else{
          itemI[item] <- CDM_item_2PL_4_mole(theta.b,a.b,d.b,Y.b,h=h)
        }
      }else if(I=="RES"){
        if(is.null(h)){
          itemI[item] <- RES_item_2PL_4_mole(theta.b,a.b,d.b,Y.b)
        }else{
          itemI[item] <- RES_item_2PL_4_mole(theta.b,a.b,d.b,Y.b,h=h)
        }
      }
    }
    itemI
  ...
  itemSE_Fisher_2PL_4_mole <- function(x,BID,Y,IN,q=10,h=NULL){
    D <- max(BID$Dim)
    ...
    for (h in 1:nrow(resorder)) {
      ...
      for(i in 1:q^D){
        for(j in 1:J){
          p1_5[i,j]=px_2PL_4_mole(X_matrix[i,],a[,j],d[,j],x=all.x[[duiying[newresorder1[i,j],1]]])+px_2PL_4_mole(X_matrix[i,],a[,j],d[,j],x=all.x[[duiying[newresorder1[i,j],2]]])
        }
      }
      paiH1[h,h]=sum(apply(p1_5, 1, prod)*AX_p)
      post=apply(p1_5, 1, prod)*AX_p/paiH1[h,h]

      sinI_vector[,h]=foreach(i = seq_len(nrow(X_matrix)),.inorder=FALSE,.combine='+',.export = c("itemI_2PL_4_mole",                                                                                                                                                                                    "grad_item_2PL_4_mole",
                                                                                                  "px_2PL_4_mole",
                                                                                                  "XPD_item_2PL_4_mole"))%dopar%{
                                                                                                    itemI_2PL_4_mole(X_matrix[i,],data.frame(a=c(x$a),d=c(x$d)),BID,resorder[h,],I='XPD',h=NULL)*post[i]
                                                                                                  }
    }
  ...
  itemSE_Simple_2PL_4_mole <- function(x,BID,Y,SE='XPD',IN,hess=NULL,h1=NULL){
    s1=Sys.time()
    D <- max(BID$Dim)
    N <- nrow(Y)  # number of examinees
    J <- ncol(Y)  # number of items
    ...
    for (h in 1:nrow(Y)) {
      ...
      if(SE=='Louis'|SE=='Sandwich'){
        g=itemI_2PL_4_mole(thetaexchoose,data.frame(a=c(x$a),d=c(x$d)),BID,as.matrix(Y[h,]),I='XPD',h=NULL)
        # 初始化结果矩阵
        ...
        sinI_vector[,h]=colMeans(itemI_2PL_4_mole(thetaexchoose,data.frame(a=c(x$a),d=c(x$d)),BID,as.matrix(Y[h,]),I=SE,h=h1))
