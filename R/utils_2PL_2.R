param.vec.2.ads_2PL_2 <- function(pv,J,D,sigm=NULL,fix.sigma=FALSE){
...
kernel_2PL_2 <- function(Y,BID,a,d,sigm,theta,B=20,J,D,N,cor.matrix=FALSE,positive,fix.sigma=FALSE,blocksize=2,hessian=FALSE){

arm.sampler_2PL_2 <- function(thetai,p){
  logLi_2PL_2(thetai, a=a, d=d,yi=Y[i,], BID=BID,prior=TRUE,sigma=sigm)
}

objfun_2PL_2 <- function(x,thetaj,yj,blocksize=2){
  IN <- 2*blocksize-1 # Number of item parameter
  -1*P.Yj_2PL_2(aj=(x[1:blocksize]),dj=c(x[(blocksize+1):IN],-x[(blocksize+1):IN]),thetaj,yj)
}

logLi_2PL_2 <- function(thetai,
                        a,
                        d,
                        yi,
                        BID,
                        prior=TRUE,
                        sigma=NULL){
  ll <- sum(log(P.Yi_2PL_2(a=a,d=d,theta=matrix(thetai[BID$Dim],nrow = 2),yi=yi)))
  lprior <- 0
  if(prior)
    lprior <- mvnfast::dmvn(X=matrix(thetai,nrow = 1),mu=rep(0,length(thetai)),sigma = sigma,log = TRUE)
  ll+lprior
}
P.Yj_2PL_2 <- function(aj,#vector of length 2
...
P.Yi_2PL_2 <- function(a,#matrix 2 x J
                       d,#matrix 2 x J
  ...
  for(b in seq_len(B)){
    # sampling theta - Equation 4
      theta <- foreach (i=seq_len(N),.combine="rbind",.errorhandling = "stop",.inorder = FALSE)%dopar%{
                          armspp::arms_gibbs(n_samples = 1,previous = theta[i,],log_pdf = arm.sampler_2PL_2,
      ...
      opt <- foreach (j=1:J,.combine="cbind",.errorhandling = "stop",.inorder = TRUE)%dopar%{
                       ...
                       optim(par=c(a[,j],d[1:(blocksize-1),j]),fn = objfun_2PL_2,method = "L-BFGS-B",lower = lo, upper = up,
      ...
      if(hessian==TRUE){
        hess_vec=foreach (j=1:J,.combine="cbind",.errorhandling = "stop",.inorder = TRUE)%dopar%{
          ...
          optim(par=c(a[,j],d[1,j]),fn = objfun_2PL_2,method = "L-BFGS-B",lower = lo, upper = up,
...
Pj_2PL_2 <- function(j,theta,aj=NULL,dj=NULL,BID=NULL,item.par=NULL){
  response.pattern <- c(1,2)
...
logL_2PL_2 <- function(theta, #vector of length D  ability for person i
                        a,#matrix 3 x J
                        d,#matrix 3 x J
                        yi,# vector of J
                        BID,
  ...
  sumz <- matrix(colSums(z)[col(z)],nrow = 2)
  p.a.ab <- z/sumz #three rows: za/sum(z),zb/sum(z); J columns
  p.y <- p.a.ab[response.pattern,][matrix(c(yi,seq_len(length(yi))),ncol = 2)]
  if(is.null(sigma)){
    # log likelihood
    logP <- sum(log(p.y))
  }else{
