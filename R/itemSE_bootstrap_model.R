...
itemSE_bootstrap_TIRT<- function(plist,blocksize,IN,J,D){
...
itemSE_bootstrap_ZG<- function(plist,blocksize,IN,J,D){
  ...
  SE.sigm[upper.tri(SE.sigm)] <- t(SE.sigm)[upper.tri(SE.sigm)]#填充上三角
  return(list(SE.a=matrix(SE.a,ncol=J,nrow=blocksize),SE.t=matrix(SE.t,ncol=J,nrow=blocksize),SE.sigm=SE.sigm))
}

itemSE_bootstrap_GGUM<- function(plist,blocksize,IN,J,D){
  ...
  for (j in 1:J) {
    ...
    end_index <- min(start_index + blocksize - 1, length(SE.all))
    extracted_part <- SE.all[start_index:end_index]
    SE.d <- c(SE.d, extracted_part)
  }
  for (j in 1:J) {
    start_index <- (j - 1) * IN + 1+blocksize*2
    end_index <- min(start_index + blocksize - 1, length(SE.all))
    extracted_part <- SE.all[start_index:end_index]
    SE.t <- c(SE.t, extracted_part)
  }
  ...
  SE.sigm[upper.tri(SE.sigm)] <- t(SE.sigm)[upper.tri(SE.sigm)]#填充上三角
  return(list(SE.a=matrix(SE.a,ncol=J,nrow=blocksize),SE.t=matrix(SE.d,ncol=J,nrow=blocksize),SE.sigm=SE.sigm))
}
