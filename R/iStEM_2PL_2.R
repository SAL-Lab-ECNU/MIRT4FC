...
iStEM_2PL_2 <- function(Y,BID,positive=rep(TRUE,nrow(BID)),M=10,B=20,SE='complete',blocksize=2,
  ...
  x <- kernel_2PL_2(Y=Y,BID=BID,a=a,d=d,sigm=sigm,theta=theta,B=B,J=J,D=D,N=N,cor.matrix = TRUE,positive=positive,fix.sigma=fix.sigma,hessian = TRUE,blocksize=blocksize)
  a <- x$a
  d <- x$d
  sigm <- x$sigm
  theta <- x$theta
  for(m in 1:M){
    ...
    cat("\n  # of batch = ",m)
    x <- kernel_2PL_2(Y=Y,BID=BID,a=a,d=d,sigm=sigm,theta=theta,B=B,J=J,D=D,N=N,cor.matrix = (m<M/2),positive=positive,fix.sigma=fix.sigma,hessian = TRUE,blocksize=blocksize)
    plist[[m]] <- x$pmatrix
...
d.hat <- batch.var(plist,n=m)
 for(i in 1:5){ # 5 iterations
    ...
    x <- kernel_2PL_2(Y=Y,BID=BID,a=a,d=d,sigm=sigm,theta=theta,B=B,J=J,D=D,N=N,positive=positive,fix.sigma=fix.sigma,hessian = TRUE,blocksize=blocksize)
    a <- x$a
    d <- x$d
    sigm <- x$sigm
    theta <- x$theta
  ...
  if(SE=='MCMC'){
    ...
    SE_item=itemSE_Simple_2PL_2(se.item,BID,Y,SE,IN,hess_mean,h1=h)
