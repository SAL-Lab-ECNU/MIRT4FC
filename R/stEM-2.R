...
stEM_2 <- function(Y,BID,positive=rep(TRUE,nrow(BID)),M=10,B=20,a=NULL,d=NULL,
  ...
  if(is.null(a)){
    if(!is.null(item.par)){
      a <- matrix(item.par$a,nrow = 2) #2 x J
    }else{
      a <- matrix(positive,nrow=2,ncol=J)
    }
  }
  #initial d parameters
  if(is.null(d)){
    if(!is.null(item.par)){
      d <- matrix(item.par$d,nrow = 2) #2 x J
    }else{
      d <- matrix(rnorm(2*J),nrow=2)
    }
  ...
  x <- kernel_2(Y=Y,BID=BID,a=a,d=d,sigm=sigm,theta=theta,B=B,J=J,D=D,N=N,cor.matrix = TRUE,positive=positive,fix.sigma=fix.sigma)
  a <- x$a
  d <- x$d
  sigm <- x$sigm
  theta <- x$theta
  for(m in 1:M){
    ...
    cat("\n  # of batch = ",m)
    x <- kernel_2(Y=Y,BID=BID,a=a,d=d,sigm=sigm,theta=theta,B=B,J=J,D=D,N=N,cor.matrix = (m<M/2),positive=positive,fix.sigma=fix.sigma)
    plist[[m]] <- x$pmatrix
    a <- x$a
    d <- x$d
    sigm <- x$sigm
...
d.hat <- batch.var(plist,n=m)
 for(i in 1:5){ # 5 iterations
    ...
    x <- kernel_2(Y=Y,BID=BID,a=a,d=d,sigm=sigm,theta=theta,B=B,J=J,D=D,N=N,positive=positive,fix.sigma=fix.sigma)
    a <- x$a
    d <- x$d
    sigm <- x$sigm
    theta <- x$theta
  ...
  est <- param.vec.2.ads_2(rowMeans(Reduce(cbind,plist)),J,D,sigm=sigm,fix.sigma=fix.sigma)
