...
iStEM_2PL_4_rank <- function(Y,BID,positive=rep(TRUE,nrow(BID)),M=10,B=20,SE='Bootstrap',blocksize=4,
  ...
  x <- kernel_2PL_4_rank(Y=Y,BID=BID,a=a,d=d,sigm=sigm,theta=theta,B=B,J=J,D=D,N=N,cor.matrix = TRUE,positive=positive,fix.sigma=fix.sigma)
  a <- x$a
  d <- x$d
  sigm <- x$sigm
  theta <- x$theta
  for(m in 1:M){
    ...
    cat("\n  # of batch = ",m)
    x <- kernel_2PL_4_rank(Y=Y,BID=BID,a=a,d=d,sigm=sigm,theta=theta,B=B,J=J,D=D,N=N,cor.matrix = (m<M/2),positive=positive,fix.sigma=fix.sigma)
    plist[[m]] <- x$pmatrix
    pplist[[m]] <- x$theta
    a <- x$a
    d <- x$d
...
d.hat <- batch.var(plist,n=m)
 for(i in 1:5){ # 5 iterations
    ...
    x <- kernel_2PL_4_rank(Y=Y,BID=BID,a=a,d=d,sigm=sigm,theta=theta,B=B,J=J,D=D,N=N,positive=positive,fix.sigma=fix.sigma)
    a <- x$a
    d <- x$d
    sigm <- x$sigm
    #print(sigm)
  ...
  while(max(d.hat*N)>=eps2&&n<maxitr){
    ...
    hess <- kernel_2PL_4_rank(Y=Y,BID=BID,a=a,d=d,sigm=sigm,theta=theta,B=1,J=J,D=D,N=N,positive=positive,fix.sigma=fix.sigma,hessian = TRUE)$hess
    cat("\n  # of valid batch = ",n," max delta hat = ",max(d.hat)*N, " criterion = ",eps2)

  cat("\nLenth of final MC chain = ",n*B)
  total.number.of.batch <- total.number.of.batch + n - M
  est <- param.vec.2.ads_2PL_4_rank(rowMeans(Reduce(cbind,plist)),J,D,sigm=sigm,fix.sigma=fix.sigma)
  #计算SE
  SE_item=list()
  ...
  if(SE=='Bootstrap'){
    ...
    SE_item=itemSE_2PL_4_rank(est,BID,Y,model,SE,IN,theta=thetaex,h=h)
