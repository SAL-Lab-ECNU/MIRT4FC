...
stEM_4_rank <- function(Y,BID,positive=rep(TRUE,nrow(BID)),M=10,B=20,a=NULL,d=NULL,
  ...
  x <- kernel_4_rank(Y=Y,BID=BID,a=a,d=d,sigm=sigm,theta=theta,B=B,J=J,D=D,N=N,cor.matrix = TRUE,positive=positive,fix.sigma=fix.sigma)
  a <- x$a
  d <- x$d
  sigm <- x$sigm
  theta <- x$theta
  for(m in 1:M){
    ...
    cat("\n  # of batch = ",m)
    x <- kernel_4_rank(Y=Y,BID=BID,a=a,d=d,sigm=sigm,theta=theta,B=B,J=J,D=D,N=N,cor.matrix = (m<M/2),positive=positive,fix.sigma=fix.sigma)
    plist[[m]] <- x$pmatrix
    a <- x$a
    d <- x$d
    sigm <- x$sigm
...
d.hat <- batch.var(plist,n=m)
 for(i in 1:5){ # 5 iterations
    ...
    x <- kernel_4_rank(Y=Y,BID=BID,a=a,d=d,sigm=sigm,theta=theta,B=B,J=J,D=D,N=N,positive=positive,fix.sigma=fix.sigma)
    a <- x$a
    d <- x$d
    sigm <- x$sigm
    #print(sigm)
  ...
  est <- param.vec.2.ads_4_rank(rowMeans(Reduce(cbind,plist)),J,D,sigm=sigm,fix.sigma=fix.sigma)
