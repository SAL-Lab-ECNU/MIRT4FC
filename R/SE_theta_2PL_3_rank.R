px_2PL_3_rank <- function(theta,a,d,xx=c(1,2,3)){ #a=c(ai,aj,ak) d=c(di,dj,dk) theta=c(thetai,thetaj,thetak)
  z <- exp(a*theta-d)
...
Ib_theta_2PL_3_rank <- function(theta,a,d){
  ...
  for(x in 1:length(all.x)){
    ...
    ret <- ret + tcrossprod(g,g)/px_2PL_3_rank(theta,a,d,xx=all.x[[x]])-hes_theta_2PL_3_rank(theta,a,d,x=all.x[[x]])
  }
  ret
}
