...
Pj_2PL_4_rank <- function(j,theta,aj=NULL,dj=NULL,BID=NULL,item.par=NULL){
...
data.sim_2PL_4_rank <- function(item.par,theta,BID){

  Y <- matrix(NA,nrow = nrow(theta),ncol = max(BID$Block))
  for(j in unique(BID$Block)){
    pj <- Pj_2PL_4_rank(j=j,theta=theta,BID=BID,item.par=item.par)
...
kernel_2PL_4_rank <- function(Y,BID,a,d,sigm,theta,B=20,J,D,N,cor.matrix=FALSE,positive,fix.sigma=FALSE,hessian=FALSE){
# posterior for sampling theta - see Equation 4
arm.sampler_2PL_4_rank <- function(thetai,p){
  logLi_2PL_4_rank(thetai, a=a, d=d,yi=Y[i,], BID=BID,prior=TRUE,sigma=sigm)
}
# function to be optimized for estimating item parameters - see Equation 8
objfun_2PL_4_rank <- function(x,thetaj,yj){
  -1*P.Yj_2PL_4_rank(aj=(x[1:4]),dj=c(x[5:7],-sum(x[5:7])),thetaj,yj)
}
P.Yj_2PL_4_rank <- function(aj,#vector of length 4
...
P.Yi_2PL_4_rank <- function(a,#matrix 4 x J
                            d,#matrix 4 x J
...
logLi_2PL_4_rank <- function(thetai, #vector of length D  ability for person i
                             a,#matrix 4 x J
                             d,#matrix 4 x J
                             yi,# vector of J
                             BID,
                             prior=TRUE,
                             sigma=NULL){
  ll <- sum(log(P.Yi_2PL_4_rank(a=a,d=d,theta=matrix(thetai[BID$Dim],nrow = 4),yi=yi)))
  lprior <- 0
  if(prior)
  ...
  for(b in seq_len(B)){
    ...
    theta <- foreach (i=seq_len(N),.combine="rbind",.errorhandling = "stop",.inorder = TRUE)%dopar%{
                        ...
                        armspp::arms_gibbs(n_samples = 1,previous = theta[i,],log_pdf = arm.sampler_2PL_4_rank,
    ...
    opt <- foreach (j=1:J,.combine="cbind",.errorhandling = "stop",.inorder = TRUE)%dopar%{
                      ...
                      optim(par=c(a[,j],d[1:3,j]),fn = objfun_2PL_4_rank,method = "L-BFGS-B",lower = lo, upper = up,
...
param.vec.2.ads_2PL_4_rank <- function(pv,J,D,sigm=NULL,fix.sigma=FALSE){
...
logL_2PL_4_rank <- function(theta, #vector of length D  ability for person i
                            a,#matrix 3 x J
                            d,#matrix 3 x J
                            yi,# vector of J
                            BID,
  ...
  p.b.bcd <- z[response.pattern[,2],]/red.sumz[response.pattern[,1],]
  p.c.cd <- z[response.pattern[,3],,drop=FALSE]/(z[response.pattern[,3],,drop=FALSE]+z[response.pattern[,4],,drop=FALSE])
  p.y <- p.a.abcd[response.pattern[,1],]*p.b.bcd*p.c.cd[matrix(c(yi,seq_len(length(yi))),ncol = 2)]
  p.y[p.y<1e-5] <- 1e-5
  if(is.null(sigma)){
    # log likelihood
    logP <- sum(log(p.y))
