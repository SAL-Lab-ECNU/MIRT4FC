...
iStEM_2PL_3_rank <- function(Y,BID,positive=rep(TRUE,nrow(BID)),M=10,B=20,SE='Louis',blocksize=3,
  ...
  x <- kernel_2PL_3_rank(Y=Y,BID=BID,a=a,d=d,sigm=sigm,theta=theta,B=B,J=J,D=D,N=N,cor.matrix = TRUE,positive=positive,fix.sigma=fix.sigma,hessian = TRUE)
  a <- x$a
  d <- x$d
  sigm <- x$sigm
  theta <- x$theta
  for(m in 1:M){
    ...
    cat("\n  # of batch = ",m)
    x <- kernel_2PL_3_rank(Y=Y,BID=BID,a=a,d=d,sigm=sigm,theta=theta,B=B,J=J,D=D,N=N,cor.matrix = (m<M/2),positive=positive,fix.sigma=fix.sigma,hessian = TRUE)
    plist[[m]] <- x$pmatrix
...
d.hat <- batch.var(plist,n=m)
 for(i in 1:5){ # 5 iterations
    ...
    x <- kernel_2PL_3_rank(Y=Y,BID=BID,a=a,d=d,sigm=sigm,theta=theta,B=B,J=J,D=D,N=N,positive=positive,fix.sigma=fix.sigma,hessian = TRUE)
    a <- x$a
    d <- x$d
    sigm <- x$sigm
    #print(sigm)
  ...
  if(SE=='MCMC'){
    ...
    SE_item=itemSE_Simple_2PL_3_rank(se.item,BID,Y,SE,IN,hess_mean,h1=h)
    SE_item$SE.a=t(SE_item$SE.a)
    SE_item$SE.d=t(SE_item$SE.d)
  }else if(SE=='complete'){
    se <- matrix(sqrt(diag(solve(hess_mean))),nrow=max(BID$Block),ncol=IN,byrow = TRUE)
    se.a <- se[,1:3]
    se.d <- se[,4:5]
